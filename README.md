...
- 服务端会在 mTLS 握手响应及后续心跳确认中下发该设备对应的最新网关 Desired State，Agent 会回传带 revision 的应用 ACK；Agent 已将本地发布网段、远端接收路由和站点互联的 SNAT 策略整理成应用计划，但当前未接入 Tailscale 路由执行器，因此 ACK 保持 `CHECKING`，不会把未生效的路由标记为 `READY`。
