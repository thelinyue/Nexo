...
test("Server 地址规范化保留 IPv6、端口及路径，拒绝危险或歧义输入", () => {
  ...
  for (const token of ["", "   ", "first\necho bad", "bad\rvalue", "bad\0value"]) expect(() => agentDeploymentContent(template, "https://example.com", token, "compose")).toThrow();
});

test("Compose 模式直接生成可保存的配置文件", () => {
  const config = agentDeploymentContent(template, "https://nexo.example.com", "one-time-token", "compose");
  ...
  expect(() => agentDeploymentContent(template.replace(/^.*NEXO_ENROLLMENT_TOKEN:.*$/m, ""), "https://example.com", "token", "compose")).toThrow();
});
...
test("镜像标签取自 Compose，支持 latest 与数字版本", () => {
  ...
  for (const method of ["compose", "docker"] as const) {
    expect(agentDeploymentContent(numbered, "https://example.com", "token", method)).toContain("nexo-agent:1.2.3");
    expect(agentDeploymentContent(template, "https://example.com", "token", method)).toContain("nexo-agent:latest");
    expect(() => agentDeploymentContent(template.replace(/nexo-agent:\S+/, "nexo-agent"), "https://example.com", "token", method)).toThrow();
  }
  expect(agentDeploymentContent(numbered, "https://example.com", "token", "docker")).toContain("nexo-agent-1.2.3/data/nexo-agent");
});
...
test("Compose 实际解析生成配置，特殊字符保持原值", () => {
  ...
  try {
    writeFileSync(join(root, "compose.yml"), agentDeploymentContent(template, server, token, "compose"));
...
test("docker run 直接命令保留特殊字符且不执行命令替换", () => {
  ...
  const server = "https://example.com/path/'$HOME/`literal`";
  const command = agentDeploymentContent(template, server, token, "docker");
  try {
    ...
    expect(args.some(arg => arg.endsWith("/nexo-agent-latest/data/nexo-agent:/data/nexo-agent"))).toBeTruthy();
  } finally {
    if (!resolve(root).startsWith(resolve(tmpdir()) + (process.platform === "win32" ? "\\" : "/"))) throw new Error("测试清理路径越界");
    rmSync(root, { recursive: true, force: true });
  }
